...
const SideBar: FunctionComponent<SideBarProps> = ({
  ...
  return (
    <div className="col-start-1 relative w-full f1:w-2/3 h-fit f1:h-full grid grid-flow-row auto-rows-auto bg-offWhite/95 row-start-1 pt-40 pb-10 f1:overflow-y-scroll overflow-x-clip">
      <div className="relative w-full h-fit grid grid-flow-row auto-rows-auto row-start-1 self-start gap-10 justify-self-start">
        <div
          className={`relative w-fit h-fit row-start-1 pl-4 f1:pl-14 pr-4 ${
            !queryWindowSize900
                  ...
                  {mixtape?.metadata?.content
                    ...
                    ?.map((image: any, indexTwo: number) => {
                      return (
                        <HotPublication
                          height="72"
                          width={queryWindowSize900 ? "96" : "full"}
                          key={indexTwo}
                          index={indexTwo}
                          data={mixtape}
                          image={image}
