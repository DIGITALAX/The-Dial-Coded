        ...
        {(more ? presetOptions : lodash.slice(presetOptions, 0, 13)).map(
          (format: string, index: number) => {
            return (
              <span
                key={index}
                className={`relative w-fit h-fit inline-flex items-center px-3 py-1.5 rounded-full shadow-sm cursor-pointer hover:bg-offBlue text-offBlack/80 lowercase underline underline-offset-2 drop-shadow-md whitespace-nowrap place-self-center`}
              >
                {format}
              </span>
            );
