...
import { ReachViewerProps } from "./../types/slider.types";

const ReachViewer: FunctionComponent<ReachViewerProps> = ({
  reachSlider,
}): JSX.Element => {
  return (
    <div className="relative w-full h-full col-start-2 col-span-10 gap-3 overflow-x-scroll flex">
      {reachSlider?.map((uri: string, index: number) => {
        return (
          <div
            key={index}
