...
import { MixCheckCollectProps } from "../../../types/common.types";

const MixCheckCollect: FunctionComponent<MixCheckCollectProps> = ({
  handleClicked,
  valueClicked,
  label,
  row,
  ...
  return (
    ...
    >
      ...
      <div
        onClick={() => {
          valueClicked === "yes" || "no"
            ? handleClicked(valueClicked === "yes" ? "no" : "yes")
            : handleClicked(
                valueClicked[0] === "everyone" ? "only followers" : "everyone"
              );
        }}
        id="crt"
      ...
      >
        <div
          className={`relative w-full h-full grid grid-flow-row auto-rows-auto rounded-md p-1 col-start-1 ${
            valueClicked === "yes" ? "bg-offBlue" : "bg-white"
          }`}
        >
...
export default MixCheckCollect;
