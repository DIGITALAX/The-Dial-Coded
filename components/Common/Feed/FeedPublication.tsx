...
import { MediaSet, ProfileMedia } from "../types/lens.types";
...
const FeedPublication: FunctionComponent<FeedPublicationProps> = ({
  ...
  if (!publication.profile.metadata?.picture.original) {
    profileImage = <></>;
  } else if (publication.profile.metadata?.picture?.original) {
    if (publication.profile.metadata?.picture.original.url.includes("http")) {
      profileImage = publication.profile.metadata?.picture.original.url;
    } else {
      const cut = publication.profile.metadata?.picture.original.url.split("/");
      profileImage = `${INFURA_GATEWAY}/ipfs/${cut[2]}`;
    }
  } else {
    profileImage = publication.profile.metadata?.picture?.uri;
  }
  return (
    <div
      ...
      <div className="relative w-full h-fit row-start-1 grid grid-flow-col auto-cols-auto">
        <div className="relative w-fit h-fit col-start-1 grid grid-flow-col auto-cols-auto gap-3">
          <div className="relative w-fit h-fit col-start-1 self-center justify-self-start cursor-pointer hover:opacity-70 active:scale-95 self-center">
            <Image src={profileImage} width={25} height={25} alt="pfp" />
          </div>
        ...
        {publication.metadata.media?.map((image: MediaSet, index: number) => {
          ...
          return (
            <div
              ...
              onClick={() =>
                dispatch(setImageViewer({ actionOpen: true, actionImage: formattedImageURL.split("ipfs/")[1] }))
              }
            >
